# `vl` command reference

Detailed usage reference for every noun group `vl` currently implements. For a
high-level overview of the project, see the top-level [README](../README.md).
...
`vl org-members` is a peer of `vl org`, not a nested sub-group, for the same
reason `vl team-member` is a peer of `vl team` — see the Layout section below.
...
`org-members list` authenticate as the resolved identity; `org show` / `org list`
don't.
...
│   ├── org.py        vl org add | show | list | update
│   ├── org_members.py  vl org-members add | set-role | list | remove
...
│   ├── node.py       vl node create — in progress, not yet mounted on the root app
│   ├── whoami.py     vl whoami
│   └── history.py    vl history
└── lib/              shared helpers used across commands
    ├── store.py      local SQLite store (environments, orgs, identities, teams, nodes, tokens, history)
