...
2. For each node in order:
   ...
   - After the process is confirmed exited (§5.3 step 3 of that document already blocks on this), poll the control plane (`GET /nodes?orgNode=`, `cp-node-identity-spec.md` §2.4, same endpoint as `vl-node-spec.md` §5.4) until `state = DOWN`, so the stagger pause below doesn't start before the CP has actually ingested the `NodeDown` event. **This poll authenticates as the node being polled's own service account** (resolved via that node's `svc_acct_id`, `vl-node-spec.md` §4) — not the operator running `vl network stop` — which is also where `org_id` comes from (the JWT claim, not a query parameter) — same as `vl node reset`'s CP check (§4 below).
