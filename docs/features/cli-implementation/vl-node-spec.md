...
4. **Liveness check:** brief pause (`1`s — long enough to cover a real network round trip if `verilock`'s startup fails on the CP side, e.g. a rejected registration; the script's original `0.25`s was sized for catching only an immediate crash like a bad path or missing library, and was too short to catch a CP-side rejection in practice), then `proc.poll()`. If the process has already exited:
