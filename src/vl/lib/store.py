...
Note on ``environment``/``svc_acct``: while ``store.db`` remains a disposable
dev artifact (no shipped release, no persistent store other than the
author's own), a couple of schema changes to these two tables were made by
editing their original ``CREATE TABLE`` migration in place (``svc_acct``'s
``client_id`` column, and ``environment``'s ``token_url``/``schema_reg_url``)
rather than via a new forward migration. This is a deliberate, temporary
call — it means a store that predates those changes won't survive an
upgrade (delete and recreate it instead) — not an oversight. Revisit once
``store.db`` needs to survive across an upgrade for anyone.
"""
